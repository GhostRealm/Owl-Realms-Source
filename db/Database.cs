...
namespace db
{
    public partial class Database : IDisposable
    {
        ...
        public Database()
        {
            ...
            //con = new MySqlConnection("Server=198.27.110.117;Database=rotmg;uid=root;password=SunReversed");
            //_con = new MySqlConnection("Server=127.0.0.1;Database=rotmg;uid=root;password=botmaker");
//            _con = Testing
            _con = new MySqlConnection("Server=127.0.0.1;Database=rotmg;uid=root;password=botmaker;Pooling=true;MinimumPoolSize=10;maximumpoolsize=12000;Allow Zero Datetime=false");
          //              _con = new MySqlConnection("Server=72.24.160.84;Database=rotmg;uid=root;password=botmaker;Pooling=true;MinimumPoolSize=10;maximumpoolsize=12000;Allow Zero Datetime=false");
            _con.Open();
        }
...
AND characters.charId=death.chrId;";
        ...
        public Account Verify(string uuid, string password)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText =
                "SELECT id, name, rank, namechosen, verified, guild, guildRank, guildFame, banned, locked, ignored, bonuses FROM accounts WHERE uuid=@uuid AND password=SHA1(@password);";
        ...
        public Account GetAccount(int id)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText =
                "SELECT id, name, rank, namechosen, verified, guild, guildRank, guildFame, banned, locked, ignored, bonuses FROM accounts WHERE id=@id;";
        ...
        public Account GetAccount(string name)
        {
            MySqlCommand cmd = CreateQuery();
            cmd.CommandText =
                "SELECT id, name, rank, namechosen, verified, guild, guildRank, guildFame, banned, locked, ignored, bonuses FROM accounts WHERE name=@name;";
