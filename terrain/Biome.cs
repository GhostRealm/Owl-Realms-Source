...
namespace terrain
{
    internal class Biome
    {
        ...
        private Dictionary<MapNode, double> ComputeMoisture()
        {
            ...
            var max = waterNodes.Max(n => n.RiverValue.Value);
        ...
        private Dictionary<MapPolygon, double> RedistributeMoisture(Dictionary<MapNode, double> nodes)
        {
            ...
            {
                var y = (double) (sorted.Count - i)/sorted.Count;
                var x = (Math.Sqrt(1.0) - Math.Sqrt(1.0*(1 - y)));
                //double x = y * 0.8;
        ...
        private short GetBiomeGround(string biome)
        {
            switch (biome)
            {
                case "road":
        ...
        private static void BlurElevation(TerrainTile[,] tiles, double radius)
        {
            ...
            var gaussWidth = (blurDiam*2) + 1;

            var kernel = CreateKernel(gaussWidth, blurDiam);

            // Calculate the sum of the Gaussian kernel      
            double gaussSum = 0;
            for (var n = 0; n < gaussWidth; n++)
            {
                gaussSum += kernel[n];
            }
