...
namespace wServer.logic
{
    ...
    {
        private static _ ElderDraconis = Behav()
              .Init(0x7904, Behaves("Elder Black Dragon God",
                new RunBehaviors(
                ...
                loot: new LootBehavior(LootDef.Empty,
                    Tuple.Create(100, new LootDef(0, 2, 0, 2,
                        Tuple.Create(0.01, (ILoot)new ItemLoot("Lightning Bow")),
                            Tuple.Create(0.06, (ILoot)new ItemLoot("Potion of Max Dexterity"))
                        )))
                ))
            .Init(0x7899, Behaves("Elder Blue Dragon God",
                new RunBehaviors(
                    IfExist.Instance(-1, NullBehavior.Instance,
                        new RunBehaviors(
                            new QueuedBehavior(
                ...
                loot: new LootBehavior(LootDef.Empty,
                    Tuple.Create(100, new LootDef(0, 2, 0, 2,
                        Tuple.Create(0.05, (ILoot)new ItemLoot("Robe of Silk Water")),
                            Tuple.Create(0.06, (ILoot)new ItemLoot("Potion of Max Wisdom"))
                        )))
                ))
            .Init(0x7943, Behaves("Elder Red Dragon God",
                new RunBehaviors(
                    IfExist.Instance(-1, NullBehavior.Instance,
                        new RunBehaviors(
                            new QueuedBehavior(
                ...
                loot: new LootBehavior(LootDef.Empty,
                    Tuple.Create(100, new LootDef(0, 2, 0, 2,
                        Tuple.Create(0.05, (ILoot)new ItemLoot("Momento of the Fire Dragon")),
                            Tuple.Create(0.06, (ILoot)new ItemLoot("Potion of Max Vitality"))
                        )))
                ))
            .Init(0x7941, Behaves("Elder Green Dragon God",
                new RunBehaviors(
                    IfExist.Instance(-1, NullBehavior.Instance,
                        new RunBehaviors(
                            new QueuedBehavior(
                    ...
                    IfEqual.Instance(-1, 1,
                        new RunBehaviors(
                            SetAltTexture.Instance(3),
                            Once.Instance(TossEnemy.Instance(0*(float) Math.PI/180, 4, 0x7945)),
                            Once.Instance(TossEnemy.Instance(45*(float) Math.PI/180, 4, 0x7945)),
                            Once.Instance(TossEnemy.Instance(90*(float) Math.PI/180, 4, 0x7945)),
                            Once.Instance(TossEnemy.Instance(135*(float) Math.PI/180, 4, 0x7945)),
                            Once.Instance(TossEnemy.Instance(180*(float) Math.PI/180, 4, 0x7945)),
                            Once.Instance(TossEnemy.Instance(225*(float) Math.PI/180, 4, 0x7945)),
                            ...
                            new QueuedBehavior(EntityLesserThan.Instance(5, 0, 0x7945),
                                new SetKey(-1, 2)
                                )
                            )
                        ),
                ...
                loot: new LootBehavior(LootDef.Empty,
                    Tuple.Create(100, new LootDef(0, 2, 0, 2,
                        Tuple.Create(0.05, (ILoot)new ItemLoot("Armor of the Leaf")),
                            Tuple.Create(0.06, (ILoot)new ItemLoot("Potion of Max Speed"))
                        )))
                ))
            .Init(0x7945, Behaves("Elder NM Green Dragon Shield",
                new RunBehaviors(
                    Circling.Instance(4, 100, 5, 0x7941)
                    )
                ));
    }
}