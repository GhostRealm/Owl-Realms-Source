...
namespace wServer.logic
{
    ...
    {
        private static _ ElderShadowMage = Behav()
            .Init(0x7804, Behaves("The Elder Shadow Mage",
                new RunBehaviors(
                    Once.Instance(new SetKey(-1, 1)),
                
                    IfEqual.Instance(-1, 1,
                        new RunBehaviors(
                            HpLesser.Instance(25000000, new SetKey(-1, 2))
                            )
                        ),
                    ...
                    IfEqual.Instance(-1, 6,
                        new RunBehaviors(
                            Cooldown.Instance(500, new ElderDamageAll()),
                                                        new QueuedBehavior(
                                Cooldown.Instance(30000),
                                new SetKey(-1, 4))
                            )
                        ...
                        Tuple.Create(100, new LootDef(0, 1, 0, 8,
                            Tuple.Create(0.009, (ILoot)new ItemLoot("Proph's Bow of the Frozen Winds")),
                            Tuple.Create(0.01, (ILoot)new ItemLoot("Void Scroll")),
                            Tuple.Create(0.01, (ILoot)new ItemLoot("Titanium Shuriken")),
                            Tuple.Create(0.9, (ILoot)new ItemLoot("Shuriken of the Shining Star")),
                            Tuple.Create(0.01, (ILoot)new ItemLoot("Brute Scroll"))
                )))
            ));
